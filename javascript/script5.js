...
let table = document.getElementById("data4");
let row, cell;

function tableHandler(event) {
    row = table.insertRow();
    cell = row.insertCell();
    cell.textContent = response[3].account_number;
    cell = row.insertCell();
    cell.textContent = response[3].currency;
    cell = row.insertCell();
    cell.textContent = response[3].balance;
    cell = row.insertCell();
    cell.textContent = response[3].transaction_date;
    cell = row.insertCell();
    cell.textContent = response[3].value_date;
    cell = row.insertCell();
    cell.textContent = response[3].currency;
    cell = row.insertCell();
    cell.textContent = response[3].debit;
    cell = row.insertCell();
    cell.textContent = response[3].credit;
    cell = row.insertCell();
    cell.textContent = response[3].transaction_narative;
}
tableHandler()
